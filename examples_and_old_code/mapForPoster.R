#Map of ME-NH survey area for ABRCMS poster
#Ruby Krasnow
...
surveyGrid <-st_read("~/Downloads/Map") #CRS: WGS 84/EPSG 4326
...
states <-st_read("~/Downloads/stateBoundaries/US_State_Boundaries.shp") #CRS: WGS 84/EPSG 4326

leaflet(surveyGrid, options = leafletOptions(zoomControl = FALSE, legend.pos="topright")) %>%
  addProviderTiles(providers$Esri.WorldTerrain) %>%
  addPolygons(stroke=FALSE,
              color = ~ factpalArea(Area),
              fillOpacity = 1

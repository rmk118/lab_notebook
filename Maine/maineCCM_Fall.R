...
#non-sig both linear and EDM with tp=1 and tp=0
do_xmap_once(s_xmap_j, predictor="jonahWt", ID_col = "id", target="scallopWt", E_max=7, tp=1) 
...
logWtFall2<- catch %>% select(Season, Year, Region, Stratum, avgLogWt_s, avgLogWt_j, avgLogWt_r) %>% 
  filter(Season == "Fall") %>% group_by(Stratum, Region, Year) %>% 
  summarise(meanLogWt_s = mean(avgLogWt_s), meanLogWt_r = mean(avgLogWt_r), meanLogWt_j = mean(avgLogWt_j))
logWtFall2$area <- paste(logWtFall2$Region, logWtFall2$Stratum)

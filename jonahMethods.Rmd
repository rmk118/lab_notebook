...
addDirection <- function(df) {
  df_out <- df %>% 
    mutate(xmap = case_when(predator=="jonah" & prey=="scallop" & direction=="predator -> prey" ~ "jonah -> scallop",
                            ...
                            predator=="jonah" & prey=="rock" & direction=="prey -> predator" ~ "rock -> jonah")) %>% select(-c(prey, predator, direction))
  return(df_out)
}
...
```{r}
params_ccm_combos <- data.frame(predator=c("jonah", "jonah"), prey=c("scallop", "rock"))
```
...
RESULTS_ccm_aggregate <- pmap_dfr(params_ccm_combos,function(predator,prey){
  lib_vec <- paste(1, nrow(catchCCMdf_agg))
  
  #find optimal E for predicting scallop or rock from jonah - although the E may be too low, we will use the E identified for univariate prediction of the target species
  rho_E_1<- EmbedDimension(dataFrame = catchCCMdf_agg, lib = lib_vec, pred = lib_vec, 
                           columns = prey,target = prey, maxE = 7, showPlot = FALSE) 
  ...
  #find optimal E for predicting jonah from scallop or rock
  rho_E_2<- EmbedDimension(dataFrame = catchCCMdf_agg, lib = lib_vec, pred = lib_vec, 
                           columns = predator,target = predator, maxE = 7, showPlot = FALSE)
...
RESULTS_ccm_wt_aggregate <- pmap_dfr(params_ccm_combos,function(predator,prey){
  lib_vec <- paste(1, nrow(wtCCMdf_agg))
  
  #find optimal E for predicting scallop or rock from jonah
...
params_areas_ccm_combos <- expand.grid(predator=c("jonah"), prey=c("scallop", "rock"), areaInput = areaList,  stringsAsFactors = FALSE) %>% 
    mutate(areaInput = as.integer(areaInput))

# By area, each individually - CATCH
RESULTS_ccm_by_area <- pmap_dfr(params_areas_ccm_combos,function(predator,prey, areaInput){
...
ggplot(data = RESULTS_ccm_by_area %>% ungroup() %>% group_by(LibSize, area) %>% summarise(across(all_of(combos), ~mean(.x, na.rm = TRUE))) %>% pivot_longer(cols = 3:6) %>% mutate(Region = substr(area, 1, 1), Stratum = substr(area, 2, 2)) %>% arrange(area))+geom_line(aes(x=LibSize, y=value, color=name))+facet_grid(Region~Stratum)
```

Just from a preliminary visual analysis, we can see clear spatial heterogeneity in the cross-map results. For example, jonah:scallop shows high predictive skill at max lib size in 22 and 44, but essentially zero predictive skill in the other areas. As one might expect from the aggregate CCM, jonah:rock shows the strongest relationship of the four cross-mappings for most areas, and areas where jonah:scallop was not as strong (e.g., areas 11 and 42) did not show strong relationships for any of the combinations we tested. Stratum (column) 3 is also notable for the close clustering of all four cross-map relationships around 0, while the four relationships generally diverge from each other with increasing library size for areas in the other three strata.
...
RESULTS_ccm_wt_by_area <- pmap_dfr(params_areas_ccm_combos,function(predator,prey, areaInput){
...
ggplot(data = RESULTS_ccm_wt_by_area %>% ungroup() %>% group_by(LibSize, area) %>% summarise(across(all_of(combos), ~mean(.x, na.rm = TRUE))) %>% pivot_longer(cols = 3:6) %>% mutate(Region = substr(area, 1, 1), Stratum = substr(area, 2, 2)) %>% arrange(area))+geom_line(aes(x=LibSize, y=value, color=name))+facet_grid(Region~Stratum)
```